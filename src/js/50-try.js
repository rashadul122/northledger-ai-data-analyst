/* "Try it on your own file": the visitor's CSV through the NorthLedger engine, in their browser.
   ...
   (one shared block, tools/check_try_guard.js checks the copies match) and the page itself fills
   every figure and grade from the engine's findings. The engine's story stays on screen. */
(function () {
  'use strict';
  ...
  var TG = (function () {
    ...
    var CODES = ['empty', 'length', 'format', 'unknown_marker', 'digit', 'percent', 'currency', 'number_word', 'grade_word',
      ...
      'binding', 'grade_swap', 'grade_missing', 'inferential', 'direction', 'unit', 'none_confirmed'];
    ...
    var CAUSE_PHRASES = ['due to', 'led to', 'lead to', 'leads to', 'leading to', 'as a result', 'resulted in', 'results in',
      ...
      'linked to', 'tied to', 'because of', 'weighed on', 'associated with'];
    ...
    var TOKEN = /\{([FS])(\d{1,3})\.([a-z]+\d{0,3})\}|\{NONE_CONFIRMED\}|\[([FS])(\d{1,3})\]|\[(?:value [A-Z]{1,3}|your file)\]/g;
    ...
    function sources(p) {
      ...
      for (var i = 0; i < fs.length; i++) {
        var f = fs[i] || {};
        out.push(source('F' + (i + 1), typeof f.claim === 'string' ? f.claim : '', has(GRADES, f.verdict) ? GRADES[f.verdict] : null));
      }
    ...
    // the tokens in normalised text, each resolved against the sources (bad: not a real slot)
    function tokens(t, byRef) {
      var out = [], m, known = Object.create(null);
      for (var r in byRef) for (var id in byRef[r].idents) known[id] = true;
      TOKEN.lastIndex = 0;
      while ((m = TOKEN.exec(t)) !== null) {
        ...
        } else if (m[0] === '{NONE_CONFIRMED}') tok.type = 'none';
        else tok.type = 'ph';
        out.push(tok);
      }
    ...
    function check(text, payload, register) {
      ...
      var add = function (c) { bad[c] = true; };
      var result = function () { return CODES.filter(function (c) { return bad[c]; }); };
      if (!raw.trim()) return ['empty'];
      if (raw.length > MAX_CHARS) { add('length'); return result(); }
      if (countWords(raw) > R.hardMaxWords) add('length');
      var t = nfkc(raw), srcs = sources(payload), byRef = Object.create(null), i;
      for (i = 0; i < srcs.length; i++) byRef[srcs[i].ref] = srcs[i];
      var findings = srcs.filter(function (s) { return s.kind === 'F'; });
      var toks = tokens(t, byRef), masked = '', last = 0;
      for (i = 0; i < toks.length; i++) {
        masked += t.slice(last, toks[i].start) + String.fromCharCode(PUA + i);
        last = toks[i].end;
        if (toks[i].bad) add('unknown_marker');
      }
      masked += t.slice(last);
      ...
      if (/[{}[\]]/.test(plain)) add('unknown_marker');
      if (/[*#`<>|~_!?]/.test(plain) || /^\s*[-•·]\s/m.test(plain) || /https?:|www\./i.test(plain) ||
        /(^|[^A-Za-z0-9])[A-Za-z0-9-]+\.(?:com|org|net|io|ai|co|uk|ca|example)(?![A-Za-z])/i.test(plain)) add('format');
      ...
      if (/\p{N}/u.test(plain)) add('digit');
      var caps = plain.match(/\b[IVXLCDM]{2,}\b/g) || [];
      for (i = 0; i < caps.length; i++) if (ROMAN.test(caps[i]) && !NOT_ROMAN[caps[i]]) add('digit');
      if (/(^|[^A-Za-z])(?=[lIO]*[lO])[lIO]{2,}(?![A-Za-z])/.test(plain)) add('digit');   // "lO" for 10
      if (/[%‰]/.test(plain)) add('percent');
      PERCENT_PHRASES.forEach(function (ph) { if (phraseIn(lower, ph)) add('percent'); });
      if (/\p{Sc}/u.test(plain)) add('currency');
      for (i = 0; i < ws.length; i++) {
        var w = ws[i];
        if (NUMBER_WORDS[w] || (w.length > 4 && /fold$/.test(w))) add('number_word');
        if (PERCENT_WORDS[w]) add('percent');
        if (GRADE_WORDS[w]) add('grade_word');
        if (CONFIDENCE_WORDS[w]) add('confidence');
        if (REGISTER_WORDS[w]) add('register');
        if (ECHO_WORDS[w]) add('echo');
        if (NEGATORS[w]) add('negation');
      }
      if (/n['’]t(?![A-Za-z])/i.test(plain)) add('negation');
      GRADE_PHRASES.forEach(function (ph) { if (phraseIn(lower, ph)) add('grade_word'); });
      REGISTER_PHRASES.forEach(function (ph) { if (phraseIn(lower, ph)) add('register'); });
      ECHO_PHRASES.forEach(function (ph) { if (phraseIn(lower, ph)) add('echo'); });
      CONFIDENCE_PHRASES.forEach(function (ph) { if (phraseIn(lower, ph)) add('confidence'); });
      if (PERSON.test(fold(plain)) || /<<<|>>>/.test(t)) add('echo');
      ...
      for (var si = 0; si < sents.length; si++) {
        ...
        if (!(noneHere && !bare) && (!nCited || !nSlots)) add('citation');
        ...
        for (i = 0; i < sw.length; i++) {
          var v = sw[i];
          if (CAUSE_WORDS[v] && !scopeWords[v]) add('cause');
          if (INTENSITY_WORDS[v] && !scopeWords[v]) add('intensity');
          if (PREDICT_WORDS[v] && !scopeWords[v]) add('prediction');
          if (ADVICE_WORDS[v] && !scopeWords[v]) add('advice');
          if (SOFT_CONFIDENCE[v] && !scopeWords[v]) add('confidence');
          if (CURRENCY_WORDS[v] && !scopeWords[v]) add('currency');
        }
        var slower = wordsOf(fold(s.replace(PUA_ALL, ' '))).join(' ');
        CAUSE_PHRASES.forEach(function (ph) { if (phraseIn(slower, ph) && !phraseIn(scopeLower, ph)) add('cause'); });
        PREDICT_PHRASES.forEach(function (ph) { if (phraseIn(slower, ph) && !phraseIn(scopeLower, ph)) add('prediction'); });
        // an engine name (confirmed_units) must be one the cited text holds
        for (i = 0; i < items.length; i++) {
          if (items[i].tok && items[i].tok.type === 'ident' && !scope.some(function (x) { return x.idents[items[i].tok.text]; })) add('vocabulary');
        }
        // each word from the cited text (by its stem) or the short list of linking words
        if (nCited) {
          for (i = 0; i < items.length; i++) {
            var iw = items[i].w;
            if (iw && !FREE[iw] && !NEUTRAL_MOVES[iw] && !scopeWords[iw] && !scopeVocab[stem(iw)]) add('vocabulary');
          }
        }
        ...
        gradeSlots.forEach(function (g) {
          involved.forEach(function (x) {
            if (x.kind === 'F' && x.grade !== g.grade && gradeSlots.indexOf(x) < 0) add('grade_swap');
          });
        });
        ...
        for (var q = 0; q < slotItems.length; q++) {
          ...
          if (!cited[sl.ref]) add('citation');
          if (sl.field !== 'n' && sl.field !== 'grade') continue;
          if (involved.length > 1) {
            var owner = ownerAt(qi);
            if (owner && owner !== sl.src) {
              if (sl.field === 'grade') { if (owner.kind === 'F' && owner.grade !== sl.src.grade) add('grade_swap'); }
              else if (!owner.nums.some(function (n) { return n.fill === sl.num.fill && n.unit === sl.num.unit; })) add('binding');
            }
          }
          ...
          if (DASHES.test(btext)) add('direction');   // a sign the model put on the engine's figure
          if (/[A-Za-z0-9]$/.test(btext)) add('digit');   // letters or digits glued before the figure
          if (atext && !/^[\s.,;:)\]'"’”]/.test(atext)) add('digit');   // "{F1.n1}k", "{F1.n1}OO", "{F1.n1}x"
          if (/^\s/.test(atext) && MAGNITUDES[wordsOf(atext)[0]] && /^\s+[A-Za-z]+(?![A-Za-z])/.test(atext)) add('number_word');
          var md = moveBeside(wordsOf(bseg[bseg.length - 1]), aw);
          if (md) {
            if (sl.num.dir ? sl.num.dir !== md : (!sl.src.dirs[md] || (sl.src.dirs.up && sl.src.dirs.down))) add('direction');
          }
          var mu = unitIn(aw);
          if (mu && sl.num.unit && mu !== sl.num.unit) add('unit');   // "3 of 6 years" where the engine wrote months
          if ((sl.num.infer || inferKind(btext)) && sl.num.infer !== inferKind(btext)) add('inferential');
        }
        ...
        for (i = 0; i < items.length; i++) {
          ...
          if (best >= 0) {
            ...
            if (bn.num.dir ? bn.num.dir !== want : !bn.src.dirs[want]) add('direction');
          } else if (!scopeDirs[want]) add('direction');
        }
      }
      if (findings.length && !anyCite) add('citation');
      // every finding the text states carries its grade (its own slot, or one shared with a finding of the same grade)
      for (var mref in mentioned) if (!graded[mref]) add('grade_missing');
      ...
      if (!bad.unknown_marker) {
        ...
        srcs.forEach(function (x) { numbersIn(x.raw).forEach(function (n) { known[n] = true; }); });
        var filled = numbersIn(fill(raw, payload, register));
        for (i = 0; i < filled.length; i++) if (!known[filled[i]]) add('digit');
      }
      var hasConfirmed = findings.some(function (f) { return f.grade === 'CONFIRMED'; });
      if (hasConfirmed ? nones > 0 : nones === 0) add('none_confirmed');
      return result();
    }

    // the text with each slot replaced by the engine's own words, as parts:
    // {text} or {cite: 'F2', claim: the cited text}. Citations are for the reader to trace.
    function parts(text, payload, register) {
      var R = has(REGISTERS, register) ? REGISTERS[register] : REGISTERS.executive;
      var t = nfkc(text), srcs = sources(payload), byRef = Object.create(null), out = [], last = 0;
      srcs.forEach(function (s) { byRef[s.ref] = s; });
      var toks = tokens(t, byRef);
      var push = function (x) { if (x) { if (out.length && out[out.length - 1].text !== undefined) out[out.length - 1].text += x; else out.push({ text: x }); } };
      toks.forEach(function (k, i) {
        ...
        else if (k.type === 'ph' || k.type === 'ident') push(k.text);
    ...
    function fill(text, payload, register, opts) {
      var keep = !!(opts && opts.cite === 'keep');
      return parts(text, payload, register).map(function (p) { return p.cite ? (keep ? '[' + p.cite + ']' : '') : p.text; }).join('')
        .replace(/[ \t]+([.,;:])/g, '$1').replace(/[ \t]{2,}/g, ' ').trim();
    }

    return { GRADES: GRADES, REGISTERS: REGISTERS, CODES: CODES, sources: sources, check: check, parts: parts, fill: fill,
      countWords: countWords, numbersIn: numbersIn };
  })();
  /* ==== END TEMPLATE GUARD ==== */
  ...
  // the proxy's answer {executive, technical, model}: both parts must pass, or neither is shown
  T.checkSummaries = function (ans, body) {
    if (!ans || typeof ans !== 'object') return { ok: false, part: null, reasons: ['empty'] };
    for (var i = 0; i < T.SUMMARY_PARTS.length; i++) {
      var part = T.SUMMARY_PARTS[i], text = ans[part];
      if (typeof text !== 'string' || !text.trim()) return { ok: false, part: part, reasons: ['empty'] };
      var reasons = TG.check(text, body, part);
      if (reasons.length) return { ok: false, part: part, reasons: reasons };
    }
    return { ok: true, executive: ans.executive.trim(), technical: ans.technical.trim() };
  };
  ...
  var REASON_TEXT = { empty: 'came back empty', length: 'was over its word limit', format: 'used formatting or characters that plain prose does not need',
    ...
    confidence: 'used confidence wording', cause: 'added a cause the engine did not state', intensity: 'added size wording the engine did not use',
  ...
  U.onReady && U.onReady(function () {
      ...
      if (S.ai) {
        h += '<div class="tr-ai-sum" role="region" aria-label="AI summaries">' +
          ...
          T.SUMMARY_PARTS.map(function (part) {
            return '<h4>' + esc(TG.REGISTERS[part].title) + '</h4><div class="tr-ai-text" data-part="' + part + '">' + T.summaryHtml(S.ai[part], S.ai.body, part, S.aiRed) + '</div>';
          }).join('') + '</div>';
      }
    ...
    function askAI(go) {
      ...
      fetch(CFG.ai_proxy_url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body),
        ...
        .then(function (ans) {
          clearTimeout(timer);
          var chk = T.checkSummaries(ans, body);      // the same guard as the proxy, on what this page sent
          if (!chk.ok) {
            S.aiNote = 'The AI summaries were set aside: the ' + (chk.part || 'returned') + ' summary ' + T.reasonText(chk.reasons) + '.' + FALLBACK;
            S.ai = null;
          } else { S.ai = { executive: chk.executive, technical: chk.technical, body: body }; S.aiRed = red; S.aiNote = ''; }
          drawStory();
        })
        .catch(function (e) {
          clearTimeout(timer);
